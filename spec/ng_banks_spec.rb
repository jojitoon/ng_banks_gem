RSpec.describe NgBanks do
  ...
  it "does not return a single bank detail for wrong code" do
    bank = NgBanks.getBank("000")
    expect(bank).to eq(nil)
  end
  it "does not return a single bank detail for wrong slug" do
    bank = NgBanks.getBank_by(:slug, "000")
    expect(bank).to eq(nil)
  end
  it "does not return a single bank detail for wrong attribute" do
    bank = NgBanks.getBank_by(:slugasa, "000")
    expect(bank).to eq(nil)
  end
end
